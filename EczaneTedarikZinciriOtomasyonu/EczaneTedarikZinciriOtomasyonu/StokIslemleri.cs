    ...
    public partial class StokIslemleri : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    genelSinif.baglanti();
                    OleDbCommand cmd = new OleDbCommand("delete from Stok where BarkodNo = @barkodNo;", genelSinif.conn);
                    cmd.Parameters.AddWithValue("@barkodNo", barkodTB.Text);
                    cmd.ExecuteNonQuery();
                ...
                else if (guncelleRB.Checked)
                {
                    ...
                    if (dr.Read())
                    {
                        ...
                        {
                            OleDbCommand cmd = new OleDbCommand("UPDATE Stok SET BarkodNo=@barkodNo,UrunAdi=@urunAdi,UrunAdet=@urunAdet,UrunFiyat=@urunFiyat,SKT=@skt where BarkodNo=@barkodNo", genelSinif.conn);
                            cmd.Parameters.AddWithValue("@barkodNo", barkodTB.Text);
                            cmd.Parameters.AddWithValue("@urunAdi", urunAdiTB.Text);
                            cmd.Parameters.AddWithValue("@urunAdet", urunAdetTB.Text);
                            cmd.Parameters.AddWithValue("@urunFiyat", fiyatTB.Text);
                    ...
                    {
                        MessageBox.Show("'" + barkodTB.Text + "' barkod numaralı ürün mevcut değil");
                    }
